...
BYTE C[17][28],D[17][28],K[17][48],c,ch;
...
void DES3( BYTE* key, const int nKeyLen, BYTE *text, BYTE *mtext )
{
	...
	else if ( nKeyLen <= 16 )//16
	{
		...
		DES( key1, text, tempText1 );
		_DES( key2, tempText1, tempText2 );
		DES( key1, tempText2, mtext );
	}
	else//24
	{
		BYTE key1[8];
...
void _DES3( BYTE* key, const int nKeyLen, BYTE *text, BYTE *mtext )
{
	...
	else if ( nKeyLen <= 16 )//16
	{
		...
		_DES( key1, text, tempText1 );
		DES( key2, tempText1, tempText2 );
		_DES( key1, tempText2, mtext );
	}
	else//24
	{
		BYTE key1[8];
