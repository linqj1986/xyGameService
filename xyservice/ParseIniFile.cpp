...
string CParseIniFile::GetValue(const string& filename, const string&  strSection,const string&  strKey)
{
	string strRet="";
	...
	string strCfgPath = szPath;
	strCfgPath+=filename;
	//printf("%s\n",strCfgPath.c_str());

	map<string,string> mapCfg;
	ReadConfig(strCfgPath,mapCfg,strSection.c_str());
	map<string, string>::const_iterator mite = mapCfg.begin();
	for (; mite != mapCfg.end(); ++mite) 
	{
		if(mite->first==strKey)
			strRet=mite->second.c_str();
	}
	return strRet;
}
